package com.meamei.baseDao.mapper.mapperhelper;
...
public class SqlHelper {
    ...
    public static String updateSetColumns(Class<?> entityClass, String entityName, boolean notNull, boolean notEmpty) {
        ...
        // 支持modify date 和 create date 自动更新
        EntityColumn createDateColumn = null;
        EntityColumn modifyDateColumn = null;
        //当某个列有主键策略时，不需要考虑他的属性是否为空，因为如果为空，一定会根据主键策略给他生成一个值
        for (EntityColumn column : columnSet) {
            ...
            if (COLUMN_CREATE_DATE.equals(column.getProperty()) && createDateColumn == null) {
                createDateColumn = column;
            }
            if (COLUMN_MODIFY_DATE.equals(column.getProperty()) && modifyDateColumn == null) {
                modifyDateColumn = column;
            }
            if (!column.isId() && column.isUpdatable()) {
                ...
                } else if (column == modifyDateColumn) {
                    // 处理modify date列
                    //set create_date = now(),
                    sql.append(column.getColumn()).append(" = now(),");
                } else if (notNull) {
